"""
sweep_rho.py — Phase transition rho vs N (analytical) + effective rho per method.
Output: tests/fixedness_test/results/rho_YYYYMMDD_HHMMSS/rho_results.csv
Esegui da repo root: python experiments/sweep_rho.py
"""
...
FIELDS = ['n_patients', 'method', 'k', 'n_attributes', 'max_domain',
          'base_vars', 'aux_vars', 'total_vars', 'total_clauses_3sat',
          'rho', 'effective_rho', 'retention_ratio']
...
def main():
    ...
    # All methods from crossproduct config, or fallback list
    cp_methods = sw.get('crossproduct', {}).get('methods', [])
    methods = cp_methods if cp_methods else [
        'no_anonymization', 'mondrian_k', 'l_diversity', 't_closeness',
        'microaggregation', 'noise_addition', 'laplace_dp', 'local_dp',
        'suppression', 'randomized_response',
    ]
    ...
    out_dir = os.path.join(_HERE, '..', 'results', f"rho_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    os.makedirs(out_dir, exist_ok=True)
    csv_path = os.path.join(out_dir, 'rho_results.csv')

    print(f"\nRho sweep: {len(n_values)} N values × {len(methods)} methods | k={k}")
    print(f"Output: {csv_path}\n")

    # Load full DB once — domains reflect full vocabulary
    full_cfg = yaml.safe_load(open(CONFIG_PATH))
    full_cfg['experiment']['real']['limit'] = 0
    full_cfg['_seed'] = 42
    print("Loading full database...")
    db_full, _, _, _ = load_attack_scenario(full_cfg)
    ...
    for method in methods:
        ...
        for n in n_values:
            # Analytical rho — schema-level, independent of anonymization
            r = compute_rho_analytical(db_full, n_records=n)

            # Effective rho — requires actual anonymization
            ...
            try:
                ...
                print(f"  N={n:5d}  method={method}  ERROR: {e}")
                eff_rho = float('nan')
